...
The JSON files under `evals/` are intended for provider-backed runs. They test rental triage and rental listing matching against expected classifications. These evaluations are deliberately separate from `scripts/test.sh` because model availability and output behavior are external dependencies.
